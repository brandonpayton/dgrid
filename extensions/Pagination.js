	...
	return declare(_StoreMixin, {
		...
		renderQueryResults: function(results, beforeNode, options){
			...
			if(!beforeNode){
				...
				Deferred.when(results.data, function(){
		...
		gotoPage: function(page, focusLink){
			...
			var result = this._trackError(function(){
				...
				rangeCollection = grid._renderedCollection.range(options.start, options.start + options.count);

				Deferred.when(grid.renderQueryResults(rangeCollection, null, options), function(rows){
					cleanupLoading(grid);
